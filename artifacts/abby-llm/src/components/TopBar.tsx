...
export default function TopBar() {
  ...
  return (
    <div className="flex items-center h-11 px-4 gap-4 flex-shrink-0 select-none border-b border-white/5">
      {/* Logo */}
      <div className="flex items-center gap-2 mr-2">
        ...
        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[hsl(var(--abby-violet))]/20 text-[hsl(var(--abby-violet))] font-medium border border-[hsl(var(--abby-violet))]/30">v1.0.0</span>
      </div>
      ...
      {["Projects", "Agents", "Models", "Datasets", "Training", "Marketplace"].map((tab, i) => (
        <button
          key={tab}
          className={`text-[12px] transition-colors px-1 ${i === 2 ? "tab-active font-medium" : "text-muted-foreground hover:text-foreground"}`}
        >
          {tab}
        </button>
      ))}
      ...
      <div className="flex items-center gap-1 ml-2">
        <button className="nav-icon-btn w-7 h-7"><Minus className="w-3 h-3" /></button>
        <button className="nav-icon-btn w-7 h-7"><Square className="w-2.5 h-2.5" /></button>
        <button className="nav-icon-btn w-7 h-7 hover:text-red-400"><X className="w-3 h-3" /></button>
      </div>
    </div>
  );
}
